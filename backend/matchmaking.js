...
function attach(server) {
  ...
  function applyLiveSquad(ctx, side, formation, starters) {
    ctx.squads[side] = {
      ...
      devotion: ctx.squads[side].devotion || {},
      captain: ctx.squads[side].captain || null,
      viceCaptain: ctx.squads[side].viceCaptain || null,
      roles: ctx.squads[side].roles || {},
    };
  ...
  function finishMatch(ctx) {
    ...
    // the new events in there but never touches result.timeline.
    const playedEvents = [...ctx.byMinute.values()].flat();
