...
// Zero-dependency static file server for the built React app (`npm run
// build` output). Same server as the vanilla frontend used — just pointed
// at dist/ instead of the source directory.
...
// new filename), so they can be cached hard — without this, every re-render
// that recreates an <img> forces a real network re-fetch of every card
// photo instead of an instant cache hit. HTML/JS/CSS aren't content-hashed
// in a way this server tracks, so they stay revalidate-on-use (via
// Last-Modified/304s) so a deploy is visible immediately instead of being
