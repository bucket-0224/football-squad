...
function computeRatings(squad) {
  ...
  slots.forEach((slotPos, i) => {
    ...
    const score = roleScore(p.attrs, slotLine) * chem;
...
function scorerFor(ratings) {
  ...
  return r ? r.player.name : '알 수 없는 선수';
}
...
function simulateMatch(homeSquad, awaySquad, homeName, awayName, opts = {}) {
  ...
  const addFlavor = (side, ratings, oppGKrating, xgVal) => {
    ...
    uniqueMinutes(n, fromMinute + 1).forEach((minute) => {
      ...
      if (roll < 0.26) {
        ...
        // VAR: the ball goes in but the goal is chalked off (offside)
        type = 'disallowed';
        text = `${who}의 골... VAR 판독 결과 오프사이드 — 득점 취소!`;
      } else if (roll < 0.46) {
        type = 'miss';
  ...
  const addGoals = (count, side, ratings) => {
    uniqueMinutes(count, fromMinute + 1).forEach((minute) => {
      ...
      events.push({ minute, type: 'goal', team: side, player: scorerFor(ratings), via });
    });
  };
  ...
  const timeline = events.map((e) => {
    ...
    if (e.type === 'goal') {
      ...
      text = `⚽ ${e.player} 골!${suffix} (${scorerTeam})`;
    }
    return {
      minute: e.minute,
      type: e.type,
...
module.exports = { computeRatings, simulateMatch, simulateRemainder, TACTICS };
