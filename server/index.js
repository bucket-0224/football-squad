...
const { computeRatings, TACTICS } = require('./game/simulate');
...
// Squad + the user's 강화 levels, so ratings reflect enhanced cards.
function withUpgrades(u, squad) {
  return { ...squad, upgrades: u.upgrades || {} };
}
...
app.post('/api/register', async (req, res) => {
  ...
  const user = {
    ...
    pvpSquad: { formation: DEFAULT_FORMATION, starters: new Array(11).fill(null), tactic: 'balanced' },
  };
  store.putUser(user);
...
app.put('/api/squad', auth.authMiddleware, (req, res) => {
  const { formation, starters, tactic, kind } = req.body || {};
  ...
  req.user[target] = { formation, starters, tactic: tactic || prev.tactic || 'balanced' };
...
app.post('/api/squad/auto', auth.authMiddleware, (req, res) => {
  ...
  const target = isPvp ? 'pvpSquad' : 'squad';
  req.user[target] = { formation, starters, tactic: squad.tactic || 'balanced' };
  store.putUser(req.user);
  res.json({ user: sanitizeUser(req.user) });
});
...
// personal stage (signing bonus). See server/transfer.js.
...
app.post('/api/market/sell', auth.authMiddleware, (req, res) => {
  ...
  req.user.coins += Math.round(price * SELL_RATE);
  store.putUser(req.user);
  res.json({ user: sanitizeUser(req.user) });
});
